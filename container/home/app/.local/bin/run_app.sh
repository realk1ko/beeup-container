...
then
  wine64 "${INSTALL_PATH}/asetlic.exe" "-p${INSTALL_PATH}/license.ini" "-L${INSTALL_PATH}/alicdat.ini" -cBOCGmbH "-l${ADOXX_LICENSE_KEY}"
fi

# if no database file exists, create it
if [ ! -f "${ADO_SQLITE_DBFOLDER}/beeup.sqlite3" ]
then
  wine64 "${INSTALL_PATH}/adbinst.exe" -dbeeup "-l${ADOXX_LICENSE_KEY}" -sSQLITE -iNO_SSO -lang2057 -c+ "-sample${INSTALL_PATH}/adostd.adl"
fi
